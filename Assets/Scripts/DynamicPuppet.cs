...
public class DynamicPuppet : MonoBehaviour
{
    ...
    {
        var dest = new Vector3(Random.value * 2 - 1, 0, Random.value * 2 - 1) + transform.localPosition;
    ...
    void Update()
    {
        ...
        if (_stepTime > 1)
        {
            if ((_stepCount & 1) == 0)
            {
                _rightFootPos = _newRightFootPos + transform.localPosition;
                _newRightFootPos = GetNextStepPos(_newLeftFootPos, _rightFootPos);
            }
            else
            {
                _leftFootPos = _newLeftFootPos + transform.localPosition;
    ...
    void OnAnimatorIK(int layerIndex)
    {
...

        {
            ...
            var up = (new Vector3(transform.localPosition.x, 1f, transform.localPosition.z) * _stepHeight + _bodyNoise.Vector(10) * 0.3f) * Mathf.Max(0, Mathf.Sin(rad));
...

        {
            ...
            var up = (new Vector3(transform.localPosition.x, 1f, transform.localPosition.z) * _stepHeight + _bodyNoise.Vector(11) * 0.3f) * Mathf.Max(0, Mathf.Sin(rad + Mathf.PI));
...

        {
            ...
            var up = new Vector3(transform.localPosition.x, 1f, transform.localPosition.z) * (_bodyHeight + Mathf.Cos(rad * 2) * _stepHeight / 2);
