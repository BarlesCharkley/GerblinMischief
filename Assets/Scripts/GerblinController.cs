...
public class GerblinController : MonoBehaviour
{
    ...
    [SerializeField] private float speed = 1.5f;              // a public setting for overall animator animation speed
    [SerializeField] private float turnSpeed = 5f;
    [SerializeField] private float lookSmoother = 3f;             // a smoothing setting for camera motion
    ...
    void FixedUpdate()
    {
        float h = Input.GetAxis("Horizontal" + playerNum);              // setup h variable as our horizontal input axis
        float v = Input.GetAxis("Vertical" + playerNum);                // setup v variables as our vertical input axis
        direction = new Vector3(Input.GetAxisRaw("Horizontal" + playerNum), 0, Input.GetAxisRaw("Vertical" + playerNum));

        float updatedSpeed = speed * speedMult * Time.fixedDeltaTime;
        //Debug.Log(updatedSpeed);
        ...
        // Create a vector in the direction the gerblin is facing with a magnitude based on the input, speed and the time between frames.
        Vector3 movement = transform.forward * updatedSpeed * direction.normalized.magnitude;

        // Apply this movement to the rigidbody's position.
        rb.MovePosition(rb.position + movement);

        float animSpeed = Mathf.Abs(updatedSpeed * direction.normalized.magnitude * 10f);
        anim.SetFloat("Speed", animSpeed);                          // set our animator's float parameter 'Speed' equal to the vertical input axis
        ...
        {
            if (Input.GetButtonUp("Jump" + playerNum))
            {
